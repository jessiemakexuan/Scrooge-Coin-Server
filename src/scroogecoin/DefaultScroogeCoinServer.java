...
import static hw3.ScroogeCoinServer.SIGNATURE_ALGORITHM;
...
public class DefaultScroogeCoinServer implements ScroogeCoinServer {
	...
	public synchronized List<HashPointer> epochHandler(List<Transaction> txs)  {
		//unordered 10min check it   accept 
	...
	public synchronized boolean isValid(Transaction tx) {
		...
		switch(trans.getType()){
			case Create:
				if(trans.numInputs()>0) return false;  //no inputs
				for(Transaction.Output op : trans.getOutputs()){
					if(op.getValue()<=0) return false;  //outputs are positive
					if(op.getPublicKey()!=scroogeKeyPair.getPublic()) return false; // all outputs are s pk
				}
				//verify it
				try {
			...
			case Pay:
				...
				for(int i=0;i<trans.numInputs();i++){
					...
					if(indexofledger==-1) return false;
